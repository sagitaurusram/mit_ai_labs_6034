...
BROTHER = IF(AND('parent (?x) (?y)','parent (?x) (?z)','male (?y)',NOT('same_identity (?y) (?z)')),THEN('brother (?y) (?z)'))
SISTER = IF(AND('parent (?x) (?y)','parent (?x) (?z)','female (?y)',NOT('same_identity (?y) (?z)')),THEN('sister (?y) (?z)'))
...
COUSIN    = IF(AND('parent (?x) (?a)','parent (?y) (?b)',OR('brother (?x) (?y)','sister (?x) (?y)')),THEN('cousin (?a) (?b)'))
GRAND_PARENT = IF(AND('parent (?a) (?b)','parent (?b) (?c)'),THEN('grandparent (?a) (?c)'))
GRAND_CHILD = IF(AND('parent (?a) (?b)','parent (?b) (?c)'),THEN('grandchild (?c) (?a)'))

#remove same identity

#MALE = IF('male (?x)', THEN(''), DELETE('male (?x)'))
#FEMALE = IF('female (?x)',THEN(), ('female (?x)'))
#PARENT = IF(AND('parent (?x) (?y)'),THEN(),('parent (?x) (?y)'))
# Then, put them together into a list in order, and call it
# family_rules.
                  # fill me in

family_rules = [SAME_IDENTITY,BROTHER,SISTER,MOTHER,FATHER,SON,DAUGHTER,COUSIN,GRAND_PARENT,GRAND_CHILD ]
...
WHAT_I_FOUND_INTERESTING = 'difficulty and the problem itself serving some purpose'
